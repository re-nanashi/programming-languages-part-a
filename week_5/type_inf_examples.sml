...
(*
  sum : T1 -> T2 [must be a function; all functions takes one argument]
  ...
  0 : int, so case-expression : int, so body : int, so T2(return type) = int
  ...
  from that and T2 = int, we know f : int list -> list
*)
...
(*
  ...
  broken_sum : T1 -> T2 [must be a function; all functions takes one argument]
  xs : T1 [must have type of f's argument]
